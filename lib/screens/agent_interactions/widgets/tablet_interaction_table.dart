...
class TabletInteractionTable extends StatelessWidget {
  final List<InteractionData> interactionData;
  const TabletInteractionTable({super.key, required this.interactionData});

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        _buildTabletTableHeader(context),
        ...interactionData.map((interactionData) => _buildRow(interactionData)),
      ],
    );
  }

  _buildTabletTableHeader(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
              flex: 1,
              child: Text("Status", style: TableHelper.getHeaderStyle())),
          Expanded(
              flex: 2,
              child: Text("Preivew", style: TableHelper.getHeaderStyle())),
          Expanded(
              flex: 1,
              child: Text("Estimate", style: TableHelper.getHeaderStyle())),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            ...
            child: Column(
              ...
              children: [
                Padding(
                  padding: const EdgeInsets.only(right: 16.0),
                  child: LenderStatusBadge(type: interactionData.status),
                ),
              ],
            ),
          ),
          ...
          Expanded(
            flex: 1,
            child: TableHelper.buildEstimateBtn(onPressed: () {}),
          ),
        ],
      ),
    );
